...
组合命令先搜索，再把前几条结果中的每个 `answer` 按搜索顺序交给详情读取：
...
# 每篇最多显示 2000 字
opencli zhihu-mobile search-answers "AI Agent" `
  --limit 10 `
  --max-content 2000
```
...
命令先产生一次搜索请求，再严格串行打开每个回答。专栏 `article`、用户 `people` 等非回答结果会被明确排除；如果前 `--limit` 条中没有回答，命令返回 `EMPTY_RESULT`。当前组合命令不提供 `remote` 源，也不会回退 Chrome。
...
opencli zhihu-mobile search-answers "AI" `
  --source fixture `
  --limit 3 `
  -f json
```
...
opencli zhihu-mobile search-answers "AI" --limit 5 -f json
```
...
opencli zhihu-mobile search-answers --help
...
opencli verify zhihu-mobile/search-answers
```
